...
class UDPHandler(socketserver.BaseRequestHandler):
       ...
       # logging.debug("-----------------")
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                f.write("Hovor zacal: "+str(origin) + " -> " + str(destination) + "\n")
                f.close()
                call = 1
            else:
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                if rx_cancel.search(self.data[0]):
                    f = open("log.txt", "a")
                    f.write(time.strftime("%a, %d %b %Y %H:%M:%S ", time.localtime()))
                    f.write("Vytacanie zrusene: " + str(origin) + " -> " + str(self.getDestination())+"\n")
                    f.close()
                    call = 0
                if rx_bye.search(self.data[0]):
                    f = open("log.txt", "a")
                    f.write(time.strftime("%a, %d %b %Y %H:%M:%S ", time.localtime()))
                    f.write("Hovor ukonceny: " + str(origin) + " -> " + str(self.getDestination()) + "\n")
                    f.close()
                    call = 0
            else:
    ...
    def processCode(self):
        ...
        if self.data[0].split(' ')[1] == '180':
            self.data[0] = self.data[0].replace("Ringing","Cinka")
        if self.data[0].split(' ')[1] == '486':
            self.data[0] = self.data[0].replace("Busy here", "Obsadene")
            f = open("log.txt", "a")
            f.write(time.strftime("%a, %d %b %Y %H:%M:%S ", time.localtime()))
            f.write("Hovor neprijaty: " + str(origin) + " ->: " + str(self.getDestination()) + "\n")
            f.close()
        if self.data[0].split(' ')[1] == '603':
            f = open("log.txt", "a")
            f.write(time.strftime("%a, %d %b %Y %H:%M:%S ", time.localtime()))
            f.write("Hovor odmietnuty: " + str(origin) + " ->: " + str(self.getDestination()) + "\n")
            f.close()
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                socket.sendto(text.encode("utf-8"), claddr)
                #showtime()
                if self.data[0].split(' ')[1] == '200' and call == 1:
                    f = open("log.txt", "a")
                    f.write(time.strftime("%a, %d %b %Y %H:%M:%S ", time.localtime()))
                    f.write("Hovor prijaty: " + str(self.getDestination()) + " -> " + str(self.getOrigin()) + "\n")
                    f.close()
               # logging.info("<<< %s" % data[0])
                #logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
